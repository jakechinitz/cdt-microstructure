...
The exclusions settle where the vacuum comes from, and the answer was already written into the architecture. Two different things are called emergence of spacetime here, and they separate cleanly. The first is kinematic: every substrate cell \emph{is} a piece of geometry by construction, since the closure invariant $K^2$ measures the failure of the four oriented faces to close --- the defining condition of a geometric tetrahedron --- and the length anchor of Section~13 converts every entanglement increment into meters. No result in the program touches this. The second is dynamical selection: among the combinatorially overwhelming crumpled and branched configurations, what makes the realized vacuum a smooth, extended, four-dimensional one? Selection is a ranking job, and a ruler cannot rank. Section~23.3 is the computed statement that nothing the theory specifies for the substrate performs it.
