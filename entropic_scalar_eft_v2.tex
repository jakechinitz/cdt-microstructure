...
\indent We also test the cell model directly, on a computer simulation of dynamical spacetime. Turning on the medium's weighting leaves the background geometry undisturbed, as the theory requires. Defects placed by hand then strain the nearby capacity and bend the local geometry, while a scrambled control leaves it flat. We can also predict, ahead of the run, one number the medium should shift in the simulation; the measured shift matches the prediction across a wide range of settings, and the control matches a different prediction of its own. This is early work at a single scale, and the parts that bear most on gravity are still being measured, but the medium and a standard model of dynamical geometry are now linked by a prediction that held, not by analogy.
...
\subsection{23.4 The conditioned vacuum and the role of the Regge host}
...
The same reading legitimates the lattice host, and fixes which host it had to be. Regge calculus is the standard discrete spelling of an extended smooth four-geometry, so a Regge background implements the paper's own conditioning in the only form a Monte Carlo program can. The requirements are concrete: the substrate needs a host with tetrahedral cells for its faces to live on, dynamical curvature, no rigid preferred spatial grid, and a demonstrated extended four-dimensional phase; Regge/CDT ensembles are the known objects meeting all four, so the choice is operational rather than convenient. Among Regge ensembles, CDT is the one in which the conditioned state is a demonstrated output: an extended four-dimensional de~Sitter-like universe emerges dynamically in its established phase \citep{AmbjornJurkiewiczLoll2004,AmbjornGJL2012}. The conditioning therefore runs on machinery that produces the very state it conditions on. The host is not the theory's own vacuum in any derivational sense --- Section~23.1 withholds exactly that claim --- but neither is it inert to the substrate placed on it. The closure sector's free energy per cell, computed from $\eta_*$ and the injectivity penalty before any coupled dynamics run, contributes a predicted extensive term to the host action's volume coupling --- the leading coefficient of the effective action $S_{\rm eff}=S_{\rm host}-\log Z_{\rm label}$ obtained by integrating the labels out (Appendix~J.6) --- and the prediction has been measured: at demonstration volume the uncentered coupling must displace the equilibrium volume by $-62.2$ units relative to the centered arm, and the three-arm measurement returned $-62.8$, one percent from prediction (Appendix~J.6). The full scaling family behind that point has since been measured: eight coupling--pin pairs spanning a fifteen-fold range of predicted displacements land at ratios $0.96$--$1.07$ to prediction, the $\beta$-curve shows the predicted concavity, the $1/\varepsilon$ law collapses to four percent, and the placebo pair lands on its own thirty-percent-displaced free-energy line at ratio $1.00$ (Appendix~J.6). Any extensive matter free energy shifts a cosmological-sector coupling --- the content here is not the existence of the shift but its magnitude, computed in advance from the ultraviolet specification alone. The computed curvature-sector shifts ($0.019$ from the closure weight, $\simeq0.08$ per cell from the seven-channel field) are pre-registered as phase-boundary displacements at production statistics, with the placebo table's induced curvature coupling seventy times smaller supplying the null line (Appendix~J.4). The kinship between the levels is structural rather than identical: $K^2$ measures the failure of a cell's oriented faces to close, while a Regge deficit angle measures the failure of the dihedral angles around a hinge to close. The link between them is hierarchy rather than identity --- cell-level non-closure reweights the glued tetrahedra, and integrating out the labels converts that reweighting into corrections to the host's volume- and curvature-sector couplings --- and the smallness of the curvature-sector shifts is the measured statement of how weakly the two levels currently couple. The substrate does not derive the Regge action; the exclusions stand. It dresses it, in the volume sector at a predicted size, with the curvature-sector dressing awaiting statistics.
...
Part of the microscopic picture has now been run as well as written. Coupled at full strength to a dynamical simplicial geometry, the admissibility ensemble orders its label sector as the closure weight demands and leaves the host geometry intact; cells pinned into persistent closure failure dent the capacity state and the geometry of their immediate neighborhood where a matched placebo does not; and the long-range sector is shown to require a conserved capacity current with maintenance sinks, from which Newton's form follows structurally; the current's measurement passes its pre-registered gates --- conservation exact, maintenance flux proportional to commitment, junction constant level-independent, far field unscreened over the measurable range (Section~23). The same calculations settle where the vacuum comes from: nothing the theory specifies lets the substrate rank geometries, so the smooth four-dimensional state is held by the conditioning of Postulate~III, and the background it selects carries a predicted contribution from the substrate's own free energy, measured as a scaling family whose placebo lands on its own predicted line. These dynamical results are graded plainly: the capacity mechanism replicated across seeds, the geometric magnitude carrying across-seed scatter, short-range, on a host whose phase map now shows a located candidate region and boundary awaiting confirmation. The ledger of Section~24 carries them at that grade.
...
The same transfer matrix run on the orbit-shuffled placebo table gives $c_0^{\rm plc}=+0.0003$ --- seventy times smaller, despite the placebo's \emph{larger} volume-sector coefficient ($c_1=1.20$ against the real table's $0.53$): the shuffle preserves the energy pool and destroys the closure structure, and only the structure prices curvature. This fixes the pre-registered form of the boundary-displacement measurement at the phase boundary the mapping of J.1 located. In bare-$k_0$ coordinates the coupled ensemble's boundary must shift by $+c_0\,r$, with $r=N_1/N_0$ the slice edge-per-vertex ratio measured from the runs themselves (about $+0.12$ at $r\simeq6.5$), while the placebo arm's boundary must not move ($+0.002$): a seventy-fold separation that dissociates closure \emph{structure} from energy \emph{magnitude}, where the volume-sector family of J.6 tested magnitude alone.
...
The reading follows the ladder fixed before the run. The capacity state responds at the first shell and only there: mean closure energy and collision fraction around the failure pins are displaced well outside the quoted errors from both the placebo and the vacuum, and both return to vacuum values by the second shell, the sub-lattice range the correlation length of J.4 requires. The local geometry responds through the third shell: at identical anchoring the failure pins carry higher first-shell coordination than the closed pins and systematically more cells per shell, a volume excess of $3.6\%$ at the third shell where the placebo matches the vacuum. Both responses separate real from placebo, so both are attributable to the failure content. The quoted errors are statistical accumulation errors, not yet thinned for autocorrelation; those caveats set the replication burden for the numbers. An independent seed has since run both arms --- different geometry evolution, different pin sites, identical protocol. The first-shell capacity separation reproduces and tightens under pooling: mean closure energy $1.678$ around the failure pins against $1.729$ around the closed pins and $1.732$ in the vacuum, with the collision deficit unchanged. The coordination separation reproduces in direction at roughly half the first seed's magnitude --- real across-seed variance, so the geometric magnitudes are quoted per seed with their scatter rather than pooled; further seeds are running. The pre-registered reading grades the capacity mechanism as replicated, and the geometric response as directionally replicated with across-seed magnitude scatter.
...
The corrected run, on a base with forty slices (twice the cells per slice, usable range to the fifth shell), returned the verdict. Conservation held exactly through four thousand sweeps and the field stayed anchored at $7.4198$. The screening fit is consistent with massless: $d\ln(\delta\!\cdot\!d)/dd=+0.115\pm0.072$, bounding the screening length above $3.9$ steps with the bound set by the slice radius rather than the physics, and the deficit persists to the fifth shell at a third of its first-shell strength, where the rectified instrument had it extinct. The profile exponent is $-0.65\pm0.14$ against the ideal $-1$, Green-function-like with the expected flattening at the closed-slice radius --- a larger lattice sharpens it, and nothing in it resembles screening. The side prediction landed: the level-zero rung's flux collapsed from $0.1075$ to $0.0086$, twelvefold, the vacuum ceasing to absorb exactly as the rectifier diagnosis required. The correction was right for the right reason, which is what makes this pass trustworthy where the earlier fail was not readable. The ladder held around it: $Q-Q(0)=0.166\,m$ (largest relative residual $0.25$, on a now-negligible baseline) and junction constant $4.66\pm0.42$, nine percent across the ladder, the absolute rescaling from the first instrument being the disclosed consequence of the rate-law change. The run is single-seed and its errors are autocorrelation-naive; within those caveats the transport sector's pre-registered gates are all passed: conservation exact, vacuum anchored, maintenance linear, junction constant, far field unscreened.
