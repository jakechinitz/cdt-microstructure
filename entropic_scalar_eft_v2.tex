...
\]
Here $\mu$ is computed analytically by transfer-matrix thermodynamic integration from $\eta_*$ and the injectivity penalty, with no lattice input. At the demonstration point ($\beta=1$, $\mu=2.488$, $\varepsilon=0.01$) the prediction is $\Delta N_{41}=-2.488/0.04=-62.2$. Three arms from a common clean base at $N_{41}=2000$ measure it:
