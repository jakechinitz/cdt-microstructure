...
\hline
Long-range propagation of the strain field & lattice dynamics & Conservation law derived; near-field readings measured; far-field gates in corrected-instrument run & budget-only response screens at sub-cell range; a conserved free-capacity current with maintenance sinks gives the massless graph-Poisson form, $\delta f\propto m/r$; measured at production volume: conservation exact, maintenance flux proportional to commitment ($Q-Q(0)=0.187\,m$, dressing subtracted), junction constant level-independent at nine percent; far-field verdict voided by the diagnosed vacuum-rectifier artifact, corrected (persistent-failure) instrument rerunning with pre-registered Coulomb and screening-mass gates & \S23.7, App.~J.8 \\
\hline
